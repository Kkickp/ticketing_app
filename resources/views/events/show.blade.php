<x-app-layout>
    <div class="max-w-7xl mx-auto py-8 px-6">
        ...
        <div class="mt-12">
            <h2 class="text-2xl font-bold text-gray-800 mb-6">Event Terkait</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                @foreach ($relatedEvents as $relEvent)
                    <x-event-card 
                        :title="$relEvent->judul" 
                        :date="$relEvent->tanggal_waktu" 
                        :location="$relEvent->lokasi" 
                        :price="$relEvent->tikets->min('harga') ?? 0" 
                        :image="$relEvent->gambar" 
                        :href="route('events.show', $relEvent)" 
                    />
                @endforeach
            </div>
        </div>
        ...
        <!-- Back Button -->
        <div class="mt-8">
            <a href="{{ route('home') }}#events" class="btn btn-outline btn-wide">
