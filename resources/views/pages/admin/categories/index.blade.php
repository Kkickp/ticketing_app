...
<div class="container mx-auto p-10">
    <div class="flex">
        <h1 class="text-3xl font-semibold mb-4 text-gray-800">Manajemen Kategori</h1>
        <button class="btn btn-primary ml-auto" onclick="add_modal.showModal()">Tambah Kategori</button>
    </div>
    <div class="overflow-x-auto rounded-box bg-white p-5 shadow-xs border border-gray-100 mt-4">
        <table class="table table-zebra w-full">
            ...
            <tbody>
                ...
                <tr class="hover:bg-gray-50/50 transition-colors">
                    <th>{{ $index + 1 }}</th>
                    <td class="font-medium text-gray-900">{{ $category->nama }}</td>
                    <td class="text-right">
                        ...
                        <button class="btn btn-xs bg-red-500 hover:bg-red-600 text-white border-none" onclick="openDeleteModal(this)" data-id="{{ $category->id }}">Hapus</button>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="3" class="text-center text-gray-400 py-6">Tidak ada kategori tersedia.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
<dialog id="add_modal" class="modal">
    <form method="POST" action="{{ route('admin.kategori.store') }}" class="modal-box">
        @csrf
...
<dialog id="delete_modal" class="modal">
    <form method="POST" class="modal-box">
        ...
        <div class="modal-action">
            <button class="btn btn-primary bg-red-500 hover:bg-red-600 border-none text-white" type="submit">Hapus</button>
