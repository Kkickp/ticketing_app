...
<div class="container mx-auto p-6 max-w-4xl">
    ...
    <div class="mb-6">
        ...
        <h1 class="text-3xl font-semibold text-gray-800 mt-3">Tambah Event Baru</h1>
        <p class="text-sm text-gray-500 mt-1">Lengkapi formulir di bawah ini untuk mempublikasikan event baru.</p>
    </div>

    <!-- Form Card -->
    <div class="card bg-white shadow-xs rounded-box border border-gray-100 overflow-hidden">
        <form method="POST" action="{{ route('admin.events.store') }}" enctype="multipart/form-data" class="card-body p-8 space-y-6">
            @csrf
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="judul" value="{{ old('judul') }}" placeholder="Masukkan judul event" class="input input-bordered w-full input-sm" required>
                        @error('judul')
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="lokasi" value="{{ old('lokasi') }}" placeholder="Contoh: Stadion Utama GBK" class="input input-bordered w-full input-sm" required>
                        @error('lokasi')
                            <span class="text-error text-xs mt-1 block">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="datetime-local" name="tanggal_waktu" value="{{ old('tanggal_waktu') }}" class="input input-bordered w-full input-sm" required>
                        @error('tanggal_waktu')
                            <span class="text-error text-xs mt-1 block">{{ $message }}</span>
                        @enderror
                    </div>

                    <!-- Gambar File Input with Cropping -->
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <div id="image-preview-container" class="hidden mt-4 card bg-gray-50 border border-gray-200 p-4 items-center justify-center max-w-sm rounded-lg">
                            <span class="text-xs text-gray-500 mb-2 font-medium">Pratinjau Gambar</span>
                            <img id="image-preview" src="#" alt="Pratinjau Gambar" class="max-h-48 object-cover rounded-lg shadow-sm">
                            <button type="button" onclick="removeImagePreview()" class="btn btn-xs btn-outline btn-error mt-3">Hapus Gambar</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-2 mt-6">
                    ...
                    <textarea name="deskripsi" rows="4" placeholder="Jelaskan detail mengenai konser/event ini..." class="textarea textarea-bordered w-full textarea-md" required>{{ old('deskripsi') }}</textarea>
                    @error('deskripsi')
                        <span class="text-error text-xs mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="pt-6 border-t border-gray-100">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-0.5">Konfigurasikan tipe tiket yang tersedia (minimal 1 tiket).</p>
                    </div>
            ...
            <div class="pt-6 border-t border-gray-100 flex justify-end gap-3">
                ...
                <button type="submit" class="btn btn-primary w-32 shadow-md">Simpan Event</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    function addTicketCard(tipe = 'reguler', harga = '', stok = '') {
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="tikets[${index}][harga]" value="${harga}" min="0" placeholder="Contoh: 150000" class="input input-bordered w-full input-sm pl-9" required>
                    </div>
                </div>
                <!-- Stok -->
                <div class="space-y-1.5">
